...
const publishableKey = optionalEnv('NEXT_PUBLIC_SUPABASE_ANON_KEY') || optionalEnv('NEXT_PUBLIC_SUPABASE_PUBLISHABLE_DEFAULT_KEY');
if (!serviceRoleKey && !publishableKey) {
    throw new Error('SUPABASE_SERVICE_ROLE_KEY or NEXT_PUBLIC_SUPABASE_ANON_KEY is required');
}
export const supabase = createClient(requireEnv('NEXT_PUBLIC_SUPABASE_URL'), serviceRoleKey || publishableKey, {
    auth: {
