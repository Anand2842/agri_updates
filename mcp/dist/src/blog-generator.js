...
function detectCategory(text) {
    if (/Urgent|Hiring|Vacancy|Walk-in|Job|Position|Role|Salary/i.test(text))
        return 'Jobs';
    if (/Webinar|Conference|Summit|Workshop|Speaker|Register/i.test(text))
        return 'Conferences';
    if (/Subsidy|Grant|Scholarship|Scheme|Funding|Eligibility|Apply/i.test(text))
        return 'Grants';
    if (/Warning|Alert|Advisory|Pest|Disease|Recall|Ban/i.test(text))
        return 'Warnings';
    if (/Startup|Founder|Funding round|Seed funding|Series [A-Z]/i.test(text))
        return 'Startups';
    return 'Research';
}
function extractJobDetails(text) {
    ...
    for (const [key, regexes] of Object.entries(patterns)) {
        for (const regex of regexes) {
            ...
            if (match?.[1]) {
                details[key] = match[1].trim().slice(0, 160);
                break;
            }
        }
    }
...
export function generatePostFromRawText(rawText) {
    ...
    return {
        ...
        content: paragraphs,
        category,
        keywords: Array.from(new Set(plain
            .split(/[^A-Za-z]+/)
